...
- BF13 complete (2026-06-24): fixed silent native crash on Deal in Expo Go — `isFlipBackFace` in `FlippableCard.utils.ts` was called inside two `useAnimatedStyle` worklets without the `'worklet'` directive; Reanimated's Babel plugin handles same-file functions automatically but not cross-file imports, so the native thread crashed trying to call a plain JS function. Fixed by converting to a regular `function` declaration with `'worklet'` directive. Also: deal-sound debounce (4× concurrent `sounds.deal()` calls), `SoundsProvider` driver + test added; diagnostic commits cleaned up; 54 app tests green.
